-- =============================================================================
-- V2__seed_data.sql
-- Hotel Manager -- Datos semilla para desarrollo.
-- Dialecto: PostgreSQL 16. Ejecutado por Flyway tras V1.
--
-- ================================================= CONTRATO PARA EL BACKEND ===
-- Las password_hash se insertan con el valor centinela 'BCRYPT_PENDING'.
-- El backend DEBE implementar un CommandLineRunner / DataInitializer que, DESPUES
-- de que Flyway corra, ejecute (pseudocodigo):
--
--     @Component
--     class DataInitializer {
--       @Autowired UserRepository repo;
--       @Autowired PasswordEncoder encoder; // BCryptPasswordEncoder cost 10
--       @PostConstruct  // o CommandLineRunner / ApplicationRunner
--       void patch() {
...
--             u.setPasswordHash(encoder.encode(creds.get(u.getEmail())));
--             repo.save(u);
--           }
--         }
--       }
...
-- (usando BCryptPasswordEncoder.encode(plaintext) -- coste 10).
-- Solo tras ese parche el login funciona.
--
...
-- =============================================================================
-- Nota: como las filas se insertan con IDs explicitos sobre columnas
-- IDENTITY (GENERATED BY DEFAULT), al final se resincronizan las secuencias
-- con setval() para que futuros inserts auto-generados no colisionen.
-- =============================================================================
...
-- Rooms -----------------------------------------------------------------------
-- Plantilla: todos AVAILABLE excepto 103 (CLEANING) y 302 (MAINTENANCE).
