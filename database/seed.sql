...
-- Equivalente a backend/src/main/resources/db/migration/V2__seed_data.sql.
-- Para cargar tras schema.sql:
--     psql -U hotel -d hotelmanager -f database/seed.sql
-- En el proyecto real, Flyway aplica V2 al arrancar Spring Boot; este archivo
-- existe solo para documentacion / inspeccion rapida.
...
-- Tipo Doble (id=2), precio 120.00/noche -> total = 2 * 120.00 = 240.00.
...
-- Payment ---------------------------------------------------------------------
-- Pago parcial: 100.00 COMPLETED en efectivo (CASH). Saldo restante = 140.00,
-- para que el flujo de check-out con saldo pendiente sea testeable (RN-6).
INSERT INTO payments (id, reservation_id, amount, method, status, reference, paid_at, created_by)
VALUES (1, 1, 100.00, 'CASH', 'COMPLETED', 'SEED-0001', now(), 1);
...
INSERT INTO audit_events (id, user_id, action, entity_type, entity_id, metadata) VALUES
    ...
    (2, 1, 'RESERVATION_CREATED', 'RESERVATION', 1,
     '{"guestId":1,"roomTypeId":2,"roomId":5,"nights":2,"totalAmount":"240.00"}'::jsonb);
